...
class ConfigurationManager:
    ...
    def __init__(
        ...
        """
        Initialize ConfigurationManager with data validation and schema configurations.

        Args:
            data_validation_config (Path): Path to data validation configuration file.
            schema_config (Path): Path to schema configuration file.
        """
        logger.info("Initializing ConfigurationManager")
        try:
            self.data_val_config = read_yaml(data_validation_config)
        except CustomException as e:
            logger.error(f"Error loading data validation config: {str(e)}")
            raise
